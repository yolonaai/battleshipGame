...
public class Board {
    ...
    private Map<Coordinate, Boolean> hitsMisses = new HashMap<>();
    ...
    public boolean receiveGuess(int x, int y) {
        ...
        if (ship != null) {
            ship.hit();
            hitsMisses.put(coord, true); // Mark hit
            if (ship.isSunk()) {
                System.out.println(ship.getName() + " has been sunk!");
            }
            return true;
        } else {
            hitsMisses.put(coord, false); // Mark miss
        }
        return false;
    }
    ...
    public char getDisplayCharacter(int x, int y) {
        Coordinate coord = new Coordinate(x, y);
        Ship ship = grid.get(coord);
        if (hitsMisses.containsKey(coord)) {
            return hitsMisses.get(coord) ? 'X' : 'O'; // Hit or miss
        } else if (ship != null) {
            switch (ship.getName()) {
                case "Battleship":
                    return 'B';
    ...
    public void displayBoard() {
        for (int y = 0; y < size; y++) {
            for (int x = 0; x < size; x++) {
                System.out.print(getDisplayCharacter(x, y) + " ");
            }
            System.out.println();
        }
    }
